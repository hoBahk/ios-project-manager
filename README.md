...
- Todo/Doing/Done의 상태를 이용하여 각 해당 List에 맞게 구현
- popover, sheet을 적절히 사용하여 구현
...
### 기능 구현
- 지난 작업내용(history)을 popover를 사용하여 보여줄 수 있도록 구현
- Todo/Doing/Done의 상태를 이용하여 해당 List에 상태에 맞는 리스트 표현
- 한 번 터치시 sheet을 통해 작업의 상세화면을 볼 수 있도록 구현
- 상세 보기 화면에서 Edit 버튼을 터치 시 수정할 수 있도록 구현
- 길게 터치시 popover를 사용하여 해당 작업의 상태를 변경할 수 있도록 구현
- UndoManager를 사용하여 작업취소 및 취소 되돌리기 기능을 구현하였다.

### 기타 구현
- 지역화의 개념을 이해하고 적절한 다국어 처리 구현
- 업무의 진행상태를 enum으로 정의하여 효율적으로 관리
...
### 3. popover할 때 List의 크기에 맞게 동적으로 변화실킬 순 없을까?
#### 문제점
...
#### 문제점
작업을 Udpate 하는 기능을 Undo 후 Redo 시에 Redo 기능이 되지 않는 문제점을 발견했다.
(Create, Delete, UpdateStatus 기능은 Undo, Redo 기능이 잘 작동되었다.)

#### 원인
다른 작업의 경우 한가지 요소만 변경을 한다. 
Create, Delete의 경우는 Task 타입 하나만 변경을 하고, UpdateStatus는 Task 타입의 status 프로퍼티만 변경한다.
하지만 Update 기능의 경우는 Title, description, deadline 총 세가지를 변경하도록 되어 있었다.

### 해결방안
해당 문제와 관련해서 여러 문서와 글을 찾아 보고 읽은 결과, UndoManager는 한번에 하나의 요소만 변경된다는 것을 확인할 수 있었다. 
그래서 Update 메서드의 파라미터를 Title, description, deadlined 하나씩 모두 받도록 하지 않고, Task타입 자체를 받아 변경할 수 있도록 수정하고 나니 문제가 해결되었다.
...
historyManager.registerUndo {
    self.updateTask(id: task.id, title: task.title, description: task. description, deadline: task. deadline)
}
```
